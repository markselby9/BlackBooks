...
{
    [super viewDidLoad];
    self.settingsArray1 = [[NSMutableArray alloc]initWithArray:@[@"书店首页", @"我的书店"]];
    //TODO 以书会友
    self.settingsArray2 = [[NSMutableArray alloc]initWithArray:@[@"系统设置"]];
    
    self.tableView.separatorColor = [UIColor colorWithRed:1/255.0f green:1/255.0f blue:255/255.0f alpha:0.9f];
    ...
    self.tableView.tableHeaderView = ({
        ...
        if (_currentuser){
            ...
            AVFile *imagefile = [_currentuser objectForKey:@"image"];
            if (imagefile){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0){
        ...
        if (indexPath.row == 1){
            BOOL isLoggedIn = [BBUserStatus isUserLoggedIn];
            if (!isLoggedIn){
