...
-(id)init
{
    XLFormDescriptor * formDescriptor = [XLFormDescriptor formDescriptorWithTitle:@"欢迎加入布莱克书店"];
    XLFormSectionDescriptor * section;
    formDescriptor.assignFirstResponderOnShow = YES;
    
    // Basic Information - Section
    section = [XLFormSectionDescriptor formSectionWithTitle:@"您的基本信息"];
    section.footerTitle = @"用户名注册之后不可修改，您可以使用昵称，但请填写真实有效的联系方式";
    ...
    // Name
    _nameRow = [XLFormRowDescriptor formRowDescriptorWithTag:@"name" rowType:XLFormRowDescriptorTypeText title:@"用户名"];
    ...
    section = [XLFormSectionDescriptor formSectionWithTitle:@"还有啥需要补充的吗"];
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *reg = [[UIBarButtonItem alloc]initWithTitle:@"加入书店" style:UIBarButtonItemStyleDone target:self action:@selector(submit:)];
    self.navigationItem.rightBarButtonItem = reg;
    
    UIBarButtonItem *cancelReg = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(cancelRegister:)];
...
- (IBAction)submit:(id)sender {
    ...
    [user signUpInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if (!error) {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"OK" message:@"欢迎您的加入！" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [alertView show];
