...
-(void)refreshView{
    ...
    UITableViewCell *footercell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
    if (self.currentUser){
        footercell.textLabel.text = [NSString stringWithFormat: @"当前登录用户：%@", self.currentUser.username];
    }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"settingsidentifier"];
    ...
    if (!self.currentUser){
        ...
        if (indexPath.section == 0){
            if ([cell.textLabel.text isEqualToString:@"个人资料设置"]){
                BBSetUserViewController *userVC = [[BBSetUserViewController alloc]init];
                userVC.user = self.currentUser;
                [self.navigationController pushViewController:userVC animated:YES];
            }
            else if ([cell.textLabel.text isEqualToString:@"密码修改"]){
                BBChangePasswordViewController *changeVC = [[BBChangePasswordViewController alloc]init];
                [self.navigationController pushViewController:changeVC animated:YES];
            }
//            else if ([cell.textLabel.text isEqualToString:@"邮箱暂未验证，点此验证"]){
//                
//            }
            else if ([cell.textLabel.text isEqualToString:@"我要吐槽"]){
                BBTucaoViewController *tucaoVC = [[BBTucaoViewController alloc]init];
                [self.navigationController pushViewController:tucaoVC animated:YES];
            }
            else if ([cell.textLabel.text isEqualToString:@"登出当前帐号"]){
                DXAlertView *alert = [[DXAlertView alloc] initWithTitle:@"Logout" contentText:@"是否登出当前帐户？" leftButtonTitle:@"Yes" rightButtonTitle:@"No"];
                [alert show];
                alert.leftBlock = ^() {
...
//                    [[UIApplication sharedApplication] openURL:url];
                UIAlertView *newversion = [[UIAlertView alloc]initWithTitle:@"检查更新" message:@"现在还不知道怎么查" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
                [newversion show];
            }else if (indexPath.row == 1){
                BBAboutmeViewController *aboutmeVC = [[BBAboutmeViewController alloc]init];
